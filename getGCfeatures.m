function features = getGCfeatures(im, imsegs)
% features(nseg, nf)
...
nloc = 0; % mean x-y, 10th, 90th percentile x-y, w/h, area
...
% 		fov=60;
% 	end
%     x=((1:imw)-(imw+1)/2)/(imw/2)*tan(fov/2/180*pi);
% 	y=-((1:imh)-(imh+1)/2)/(imh/2)*tan(fov/2/180*pi)*(imh/imw);
% % 	p=zeros(imh,imw,3);
% % 	for i=1:imh
% % 		p(i,:,2)=y(i);
% % 	end
...
% % 	end
% % 	p(:,:,3)=-1;
%     xim_fov=repmat(x,imh,1);
%     yim_fov=repmat(y',1,imw);
%     xvals_fov = xim_fov(spstats(s).PixelIdxList);
%     yvals_fov = yim_fov(spstats(s).PixelIdxList);
%     features(s, f+1) = mean(xvals_fov);
%     features(s, f+2) = mean(yvals_fov);
%     sxvals_fov = sort(xvals_fov);
%     syvals_fov = sort(yvals_fov);
%     features(s, f+3) = sxvals_fov(ceil(numel(sxvals_fov)/10));
%     features(s, f+4) = sxvals_fov(ceil(9*numel(sxvals_fov)/10));
%     features(s, f+5) = syvals_fov(ceil(numel(syvals_fov)/10));
%     features(s, f+6) = syvals_fov(ceil(9*numel(syvals_fov)/10));    
