...
void LLE(cv::Mat1f &X, cv::Mat1f &W, cv::Mat1i &neighbors, int N, int D, float tol, int K = 12)
{
	...
	printf("---->kd_tree building...N=%d D=%d nPts=%d",N,D,nPts);
	kdTree = new ANNkd_tree(dataPts, nPts, D);
	printf("done.\n");
	...
	*/
	// % STEP2: SOLVE FOR RECONSTRUCTION WEIGHTS
//	float tol;
	printf("--> Solving for reconstruction weights. tol=%f \n", tol);
//	if(K > D) {
//		printf("---->[note: K>D; regularization will be used]\n");
//		tol = 1e-3; // % regularlizer in case constrained fits are ill conditioned
//	} else {
...
//	}
	...
	//tol = 1e-2; // HARD SETTING TOL VALUE _SS_
	//tol = 1e-5;
//	tol = 1e-3;
